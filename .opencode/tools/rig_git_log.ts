...
export default tool({
  ...
  args: {
    max_count: tool.schema.number().optional().default(20).describe("Limit the number of commits to show."),
    oneline: tool.schema.boolean().optional().default(true).describe("Display commits using oneline formatting."),
    paths: tool.schema.array(tool.schema.string()).optional().default([]).describe("Only show commits affecting the specified paths."),
  },
  async execute(args, context) {
    ...
    return {
      title: result.status === "completed" ? "rig_git_log completed" : "rig_git_log result",
