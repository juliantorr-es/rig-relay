...
from vibe import __file__ as _vibe_file, __version__
...
def resolve_repo_root() -> Path | None:
    ...
    pkg = Path(_vibe_file).resolve().parent
...
def _package_root(path_str: str) -> Path | None:
    """Walk up from a module file to find the 'vibe' package root."""
    p = Path(path_str).resolve()
    for parent in [p] + list(p.parents):
        if parent.name == "vibe" and (parent.parent / "pyproject.toml").exists():
            return parent
    return None
...
def check_runtime_provenance() -> RuntimeProvenanceResult:
    ...
    pkg_path = str(Path(_vibe_file).resolve().parent)
...
def collect_startup_provenance() -> dict[str, str | None]:
    ...
    """
    head = _get_git_head()
    pkg = str(Path(_vibe_file).resolve().parent) if _vibe_file else None
    return {
        "package_path": pkg,
