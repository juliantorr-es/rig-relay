...
// ── Native Message Bridge (S0: typed, versioned bootstrap contract) ──
...
final class NativeMessageBridge: NSObject, WKScriptMessageHandler, @unchecked Sendable {
    ...
    // Recognized message kinds from frontend (allowlist, prevents spoofing)
    private let recognizedFrontendKinds: Set<String> = [
        "get_host_state",
        "get_bootstrap",
        "open_file_dialog",
    ...
    func configure(for webView: WKWebView) {
        ...
        // Remove any prior registration to avoid duplicate handlers
        controller.removeScriptMessageHandler(forName: "rigHostBridge")
    ...
    func userContentController(
        ...
        // Only dispatch recognized message kinds
        guard recognizedFrontendKinds.contains(kind) else {
            // Silently ignore unrecognized kinds — no spoofing
            return
        }
...
// ── Bridge Script (injected at document start) ─────────
