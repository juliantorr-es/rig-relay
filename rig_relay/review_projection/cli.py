...
def _get_git_info(repo_root: Path) -> tuple[str, str]:
    # Dummy git info for now, normally would use subprocess or pygit2
    return "confidential/ip-holdback/review-projection-compiler-v1", "HEAD"
...
def _run_projection(
    ...
    # 2. Setup engines
    classifier = ClassificationEngine(repo_root, policy_engine, manifest, public_validator)
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for file in files:
            ...
            if classification in (FileClassification.TRANSFORM_ALLOWED, FileClassification.PUBLIC_ALREADY_DISCLOSED):
                if file_path.suffix == ".py":
                    ...
                    try:
                        ...
                        receipt.excluded_path_hashes[file_hash] = "transformation_failed"
                        classification_counts[FileClassification.UNCLASSIFIED_REFUSED.value] += 1
                else:
    ...
    bundle_manifest.excluded_counts = {k: v for k, v in classification_counts.items() if k not in (FileClassification.TRANSFORM_ALLOWED.value, FileClassification.PUBLIC_ALREADY_DISCLOSED.value)}

    if not emit_bundle:
        receipt.output_status = "refused"
        receipt.residual_scan_result = "dry_run"
        print(f"Classification complete. Dry-run only. {len(files_to_zip)} files ready for transformation.")
        return
    ...
    for rel_path, content in files_to_zip.items():
        ...
        if scan_result:
            ...
            (output_dir / f"receipt_{projection_id}.json").write_text(receipt.model_dump_json(indent=2))
...
def _verify_bundle(zip_path: Path) -> None:
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            if manifest_data.get("schema_version") != "rig.review_projection.bundle_manifest.v1":
