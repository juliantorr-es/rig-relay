...
class BundleBuilder:
    ...
    def write_bundle(
        ...
        repo_root = self.output_dir.parents[2] if len(self.output_dir.parents) >= 3 else Path.cwd()
        for rel_path in files_content:
            if is_confidential_artifact_path(Path(rel_path), repo_root):
                raise ValueError("confidential_artifact_refused:review_projection_bundle")

        # 1. Bundle Manifest into the ZIP content map
        files_content["bundle_manifest.json"] = bundle_manifest.model_dump_json(indent=2)
