...
class PulseSpinner(Spinner):
    FRAMES: ClassVar[tuple[str, ...]] = ("■",)*6 + ("□",)*4
class SpinnerType(Enum):
    BRAILLE = auto(); PULSE = auto(); SNAKE = auto()
class SnakeSpinner(Spinner):
    ...
    def _is_in_bounds(self, position: complex) -> bool:
        return 0 <= position.real < self.MAP_WIDTH and 0 <= position.imag < self.MAP_HEIGHT
    def _get_direction(self) -> complex:
        if len(set(z.real for z in self._positions)) > 1 and len(set(z.imag for z in self._positions)) > 1 and self._is_in_bounds(self._positions[0] + self.current_direction):
    ...
    def next_frame(self) -> str:
        self._positions = self._next_positions(); return self.current_frame()
    def reset(self) -> None:
        self._positions = [1, 0, 1j]
_SPINNER_CLASSES: dict[SpinnerType, type[Spinner]] = {SpinnerType.BRAILLE: BrailleSpinner, SpinnerType.PULSE: PulseSpinner, SpinnerType.SNAKE: SnakeSpinner}
def create_spinner(spinner_type: SpinnerType = SpinnerType.BRAILLE) -> Spinner:
    return _SPINNER_CLASSES.get(spinner_type, BrailleSpinner)()
class SpinnerMixin:
    SPINNER_TYPE: ClassVar[SpinnerType] = SpinnerType.BRAILLE
    SPINNING_TEXT: ClassVar[str] = ""
    COMPLETED_TEXT: ClassVar[str] = ""
    _spinner: Spinner; _spinner_timer: Any; _is_spinning: bool; _indicator_widget: Static | None; _status_text_widget: Static | None
    def init_spinner(self) -> None:
        self._spinner = create_spinner(self.SPINNER_TYPE); self._spinner_timer=None; self._is_spinning=True; self._status_text_widget=None
    def start_spinner_timer(self) -> None:
        if not isinstance(self, HasSetInterval): raise TypeError("SpinnerMixin requires a class that implements HasSetInterval protocol")
        self._spinner_timer = self.set_interval(0.1, self._update_spinner_frame)
    def _update_spinner_frame(self) -> None:
        if not self._is_spinning or not self._indicator_widget: return
        self._indicator_widget.update(self._spinner.next_frame())
    def refresh_spinner(self) -> None:
        if self._indicator_widget: self._indicator_widget.refresh()
    def stop_spinning(self, success: bool = True) -> None:
        self._is_spinning = False
        if self._spinner_timer: self._spinner_timer.stop(); self._spinner_timer=None
        if self._indicator_widget:
            self._indicator_widget.update("✓" if success else "✕")
            self._indicator_widget.add_class("success" if success else "error")
        if self._status_text_widget and self.COMPLETED_TEXT: self._status_text_widget.update(self.COMPLETED_TEXT)
