"""ToolResultRuntime — tool result normalization, telemetry, and observation.

Phase 4 extraction target:
  ToolResponseMixin → ToolResultRuntime

Normalizes tool execution output, writes artifacts, emits telemetry
events, and records receipts. Replaces the mixin-based _handle_tool_response
with an explicit runtime that receives typed dependencies.
"""
...
class ToolResultRuntime:
    """Owns tool result normalization, artifact writing, and telemetry.

    Receives explicit references for all side effects.
    """
    ...
    def handle_tool_response(
        ...
        if should_artifact_tool_result(text):
            writer = ToolOutputArtifactWriter(loop.session_id)
            ...
            loop.telemetry_client.send_artifact_written(
        ...
        loop.telemetry_client.send_tool_call_finished(
            tool_call=tool_call,
            agent_profile_name=loop.agent_profile.name,
            model=loop.config.active_model,
            status=status,
            decision=decision,
            result=result,
            message_id=loop._current_user_message_id,
        ...
        loop.telemetry_client.send_tool_reasoning_trace(
            session_id=loop.session_id,
            tool_name=tool_call.tool_name,
        ...
        self.capture_model_observation(tool_call, status, duration_ms)
    ...
    def capture_model_observation(
        ...
        try:
            ...
            provider = loop.config.get_active_provider()
