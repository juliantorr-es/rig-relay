...
def render_heading(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    hlevel = min(max(block.get("level", 2), 1), 6)
    return f'<h{hlevel} id="{bid}" class="{css_cls}"{data_attrs}>{content}</h{hlevel}>'


def render_paragraph(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    return f'<p id="{bid}" class="{css_cls}"{data_attrs}>{content}</p>'


def render_callout(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
...
def render_list(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
...
def render_table(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    ...
    return f'<table id="{bid}" class="{css_cls}"{data_attrs}>\n{head}\n{tbody}\n</table>'


def render_code(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
...
def render_json(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    return f'<pre id="{bid}"><code class="language-json">{content}</code></pre>\n'


def render_evidence(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
...
def render_link(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    href = _html.escape(str(block.get("href", "")), quote=True)
    return f'<p id="{bid}" class="{css_cls}"{data_attrs}><a href="{href}">{content}</a></p>'


def render_file_ref(block: dict, content: str, title: str, bid: str, css_cls: str, data_attrs: str) -> str:
    path = _html.escape(str(block.get("path", "")))
    return f'<p class="file-ref {css_cls}" id="{bid}"{data_attrs}><code>{path}</code></p>'
...
def render_block(block: dict, doc_disc: dict | None = None) -> str:
    ...
    collapsible, collapsed, visible, css_cls, data_attrs = build_disclosure(block, doc_disc)
    ...
    return wrap_collapsible(body, bid, css_cls, data_attrs, collapsible, visible, collapsed, title, content)
