...
from rig_relay.site_renderer.safety import sanitize_url
...
def render_page(
    ...
    return template.render(
        ...
        source_artifact_paths=filter_exportable_artifact_paths(
            page_model.get("source_artifact_paths", [])
        ),
        ...
        nav_pages=[
            {**nav, "route": sanitize_url(nav.get("route", ""))}
            for nav in (nav_pages or [])
        ],
        ...
        og_image=sanitize_url(page_model.get("og_image", "")),
        ...
        og_url=sanitize_url(page_model.get("og_url", "")),
        ...
        canonical_url=sanitize_url(page_model.get("canonical_url", "")),
        theme_color=page_model.get("theme_color", "#1e3a5f"),
        robots=page_model.get("robots", "index,follow"),
        structured_data_json=page_model.get("structured_data_json", ""),
...
def render_index(
    ...
    return template.render(
        ...
        github_url=sanitize_url(site_meta.get("github_url", "")),
        relative_root=relative_root,
        nav_pages=[
            {**nav, "route": sanitize_url(nav.get("route", ""))}
            for nav in (nav_pages or [])
