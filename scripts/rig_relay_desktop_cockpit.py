...
def _run_ws_server(build_root: Path, host: str, port: int) -> None:
    """Run the WebSocket projection stream in a background thread."""

    async def _start() -> None:
        server = ProjectionWebSocketServer(build_root=build_root, host=host, port=port)
        await server.start()
...
def _open_window(ws_port: int | None) -> None:
    ...
    if ws_port is not None:
        ws_thread = threading.Thread(
            target=_run_ws_server, args=(BUILD_ROOT, "127.0.0.1", ws_port), daemon=True
        )
        ws_thread.start()
