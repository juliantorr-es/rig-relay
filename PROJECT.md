...
[DIRECTIVE] Builder Publication and Review: Local checkpoint commits preserve proven mission-owned work sets. Before final publication, the builder must run a prepublication review loop with a designated contract-auditor reviewer subagent until the candidate boundary is admitted or an external blocker is identified. Once the candidate boundary is prepublication-admitted and a named milestone grants publication authorization, the lane must push its checkpointed commits exactly once for remote review. The builder may only emit candidate statuses, a builder publication record, and a prepublication review cycle. Publication is separate from authorship, and it does not widen the lane's claim or reopen frozen work. Review tooling may inspect only the published checkpointed slice; unrelated or deferred seams remain deferred unless the released boundary becomes unsafe or false.

[DIRECTIVE] Prepublication Review Loop: The reviewer subagent may emit only `prepublication_admitted`, `prepublication_repair_required`, or `prepublication_blocked_external_dependency` during the internal review loop. The review report is append-only and the builder may not rewrite it. The builder must repair blocking findings, record a builder response, and resubmit until admission or an external blocker.

[DIRECTIVE] Reviewer Verification Record: Only an independent reviewer session may award `verified_narrow_release`, `verified_with_blocking_defects`, `downgraded_candidate_repair_required`, or `frozen_pending_integration`. The reviewer must read remote main, compare the builder publication record and prepublication review cycle to the source of truth, and record the verification result as a structured verification record. No lane may self-freeze.

[DIRECTIVE] Decision Records, Not Essays: Builders and reviewers record evidence, alternatives, proof, and verdicts as structured fields. They do not use prose summaries as the authoritative release artifact. If a human-readable note is included, it must still report the pushed SHA, review round count, and remote verification status.
