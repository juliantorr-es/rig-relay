...
class TestFrontendButtons:
    ...
    def test_frontend_has_only_safe_intent_buttons(self):
        ...
        # Safe intent buttons are generated by widgets.js, not hardcoded in index.html
        widgets_path = (
        ...
        for btn in safe_buttons:
            assert btn in frontend, f"Safe button {btn} not found in frontend JS"
        ...
        for btn in protected_buttons:
            assert btn not in frontend, f"Protected button {btn} found in frontend JS"

        # Verify mutation-only features are not exposed in frontend JS
        assert "ruff_format_fix" not in frontend
        assert "allow_mutation" not in frontend
