...
def test_require_path_within_workdir_rejects_outside_path(tmp_path):
    ...
    with pytest.raises(
        ToolError, match="Security error: Path .* is outside the project directory"
    ):
...
def test_require_path_within_workdir_allows_scratchpad(monkeypatch):
    ...
    monkeypatch.setattr(
        scratchpad, "_active_scratchpads", {"test": Path("/tmp/vibe/scratch")}
    )

    assert (
        require_path_within_workdir(scratch_path, workdir=Path("/home/user/project"))
        == scratch_path
    )
