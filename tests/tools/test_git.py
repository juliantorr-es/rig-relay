...
from vibe.core.tools.builtins.git import (
    ...
    GitShow, GitShowArgs,
    GitLsFiles, GitLsFilesArgs,
    GitResult
)

@pytest.fixture
def status_tool():
    return GitStatus(config_getter=lambda: GitStatus.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())

@pytest.fixture
def diff_tool():
    return GitDiff(config_getter=lambda: GitDiff.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())

@pytest.fixture
def log_tool():
    return GitLog(config_getter=lambda: GitLog.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())

@pytest.fixture
def branch_tool():
    return GitBranch(config_getter=lambda: GitBranch.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())

@pytest.fixture
def show_tool():
    return GitShow(config_getter=lambda: GitShow.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())

@pytest.fixture
def ls_files_tool():
    return GitLsFiles(config_getter=lambda: GitLsFiles.create_config_with_permission(ToolPermission.ALWAYS), state=BaseToolState())
...
async def test_git_status_argv(status_tool, monkeypatch):
    ...
    async def mock_run_git(operation, args):
        ...
        return GitResult(operation=operation, argv=recorded_argv[-1], stdout="", stderr="", returncode=0, truncated_stdout=False, truncated_stderr=False)
...
async def test_git_diff_argv(diff_tool, monkeypatch):
    ...
    await collect_result(diff_tool.run(GitDiffArgs(paths=["file.txt"], cached=True, stat=True)))
...
async def test_git_log_argv(log_tool, monkeypatch):
    ...
    await collect_result(log_tool.run(GitLogArgs(max_count=50, oneline=True, paths=["dir"])))
...
@pytest.mark.asyncio
async def test_git_rejects_absolute_paths_outside_workdir(diff_tool, tmp_path, monkeypatch):
...
async def test_git_ls_files_argv(ls_files_tool, monkeypatch):
    ...
    await collect_result(ls_files_tool.run(GitLsFilesArgs(others=True, modified=True, deleted=True, paths=["src"])))
    assert recorded_argv[0] == ["git", "ls-files", "--others", "--modified", "--deleted", "--", "src"]
