...
"""Stage 4 test snippets — appended to test_validate.py."""


def test_parse_git_status_branch_sets_branch() -> None:
    """_parse_git_status_branch sets branch from ## branch...upstream line."""
    from vibe.core.tools.builtins.validate import (
        ValidateGitState,
    ...
    _parse_git_status_branch(state, "## main...origin/main")
    assert state.branch == "main"
    assert state.upstream == "origin/main"


def test_parse_git_status_branch_with_ahead_behind() -> None:
    """_parse_git_status_branch parses ahead/behind counts."""
    from vibe.core.tools.builtins.validate import (
        ValidateGitState,
        _parse_git_status_branch,
    )
    ...
    state = ValidateGitState(is_git_repo=True)
    _parse_git_status_branch(state, "## main...origin/main [ahead 3 behind 1]")
    assert state.branch == "main"
    assert state.ahead_count == 3
    assert state.behind_count == 1


def test_parse_git_status_branch_detached_head() -> None:
    """_parse_git_status_branch handles detached HEAD line."""
    from vibe.core.tools.builtins.validate import (
        ValidateGitState,
        _parse_git_status_branch,
    )
    ...
    state = ValidateGitState(is_git_repo=True)
    _parse_git_status_branch(state, "## HEAD (no branch)")
    assert state.branch is not None
    assert state.upstream is None


def test_parse_git_status_porcelain_line_untracked() -> None:
    """_parse_git_status_porcelain_line classifies ?? as untracked."""
    from vibe.core.tools.builtins.validate import (
        ValidateGitState,
    ...
    untracked: list[str] = []
    changed: list[str] = []
    _parse_git_status_porcelain_line(state, "?? new_file.py", dirty, untracked, changed)
    assert "new_file.py" in untracked
    assert "new_file.py" in dirty
    assert "new_file.py" in changed


def test_parse_git_status_porcelain_line_modified() -> None:
    """_parse_git_status_porcelain_line classifies M as modified."""
    from vibe.core.tools.builtins.validate import (
        ValidateGitState,
        _parse_git_status_porcelain_line,
    )
    ...
    untracked: list[str] = []
    changed: list[str] = []
    _parse_git_status_porcelain_line(state, " M existing.py", dirty, untracked, changed)
    assert "existing.py" in dirty
    assert "existing.py" in changed
    assert "existing.py" not in untracked


def test_parse_git_status_porcelain_parses_full_output() -> None:
    """_parse_git_status_porcelain handles multi-line porcelain output."""
    from vibe.core.tools.builtins.validate import _parse_git_status_porcelain
    ...
    state = _parse_git_status_porcelain(output, None)
    ...
    assert "modified.py" in state.dirty_paths
    assert "deleted.py" in state.dirty_paths
    assert "untracked.py" in state.untracked_paths
    assert "staged.py" in state.changed_paths


@pytest.mark.asyncio
async def test_collect_git_state_non_repo(tmp_path: Path) -> None:
    """_collect_git_state returns is_git_repo=False in a non-git directory."""
...
@pytest.mark.asyncio
async def test_collect_git_state_empty_repo(tmp_path: Path) -> None:
    """_collect_git_state captures git state in a git repo."""
    import subprocess
    ...
    (tmp_path / "initial.txt").write_text("content")
...
@pytest.mark.asyncio
async def test_collect_git_state_detects_dirty(tmp_path: Path) -> None:
    """_collect_git_state detects modified and untracked files."""
    import subprocess
    ...
    subprocess.run(["git", "add", "."], cwd=str(tmp_path), capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "base"], cwd=str(tmp_path), capture_output=True
    )
    ...
    assert state.is_git_repo
    assert state.dirty_count > 0
    assert "base.txt" in state.dirty_paths
    assert "new.txt" in state.untracked_paths


def test_check_dirty_policy_none_passes() -> None:
    """_check_dirty_policy returns None when policy is None."""
    from vibe.core.tools.builtins.validate import ValidateGitState, _check_dirty_policy

    gs = ValidateGitState(is_git_repo=True, dirty_count=5)
    result = _check_dirty_policy(gs, None, [])
    assert result is None


def test_check_dirty_policy_allow_dirty_passes() -> None:
    """_check_dirty_policy returns None when policy='allow_dirty'."""
    from vibe.core.tools.builtins.validate import (
    ...
    gs = ValidateGitState(is_git_repo=True, dirty_count=5)
    result = _check_dirty_policy(gs, DIRTY_POLICY_ALLOW_DIRTY, [])
    assert result is None


def test_check_dirty_policy_clean_fails_when_dirty() -> None:
    """_check_dirty_policy returns a reason when policy='clean' and dirty."""
    from vibe.core.tools.builtins.validate import (
        DIRTY_POLICY_CLEAN,
        ValidateGitState,
        _check_dirty_policy,
    ...
    gs = ValidateGitState(is_git_repo=True, dirty_count=3)
    result = _check_dirty_policy(gs, DIRTY_POLICY_CLEAN, [])
    assert result is not None
    assert "dirty" in str(result).lower()


def test_check_dirty_policy_clean_passes_when_clean() -> None:
    """_check_dirty_policy returns None when policy='clean' and clean."""
    from vibe.core.tools.builtins.validate import (
        DIRTY_POLICY_CLEAN,
        ValidateGitState,
        _check_dirty_policy,
    ...
    gs = ValidateGitState(is_git_repo=True, dirty_count=0)
    result = _check_dirty_policy(gs, DIRTY_POLICY_CLEAN, [])
    assert result is None


def test_check_dirty_policy_allow_listed_dirty_passes() -> None:
    """_check_dirty_policy passes when all dirty paths are allowed."""
    from vibe.core.tools.builtins.validate import (
    ...
    gs = ValidateGitState(
        is_git_repo=True, dirty_count=2, dirty_paths=["file1.py", "file2.py"]
    )
    result = _check_dirty_policy(
        gs, DIRTY_POLICY_ALLOW_LISTED_DIRTY, ["file1.py", "file2.py"]
    )
    assert result is None


def test_check_dirty_policy_allow_listed_dirty_fails() -> None:
    """_check_dirty_policy fails when some dirty paths are not allowed."""
    from vibe.core.tools.builtins.validate import (
        DIRTY_POLICY_ALLOW_LISTED_DIRTY,
        ValidateGitState,
        _check_dirty_policy,
    ...
    gs = ValidateGitState(
        is_git_repo=True, dirty_count=2, dirty_paths=["file1.py", "file3.py"]
    )
    result = _check_dirty_policy(
        gs, DIRTY_POLICY_ALLOW_LISTED_DIRTY, ["file1.py", "file2.py"]
    )
    assert result is not None
    assert "unlisted" in str(result).lower() or "file3.py" in str(result)


def test_worktree_readiness_profile_registered() -> None:
    """worktree-readiness profile is registered."""
    from vibe.core.tools.builtins.validate import get_profile

    profile = get_profile("worktree-readiness")
    assert profile is not None
    assert profile.name == "worktree-readiness"


def test_validate_receipt_git_summary_content_light() -> None:
    """ValidateReceipt git summaries have only counts, no paths."""
    from vibe.core.tools.builtins.validate import ValidateReceipt
    ...
    dumped = receipt.model_dump(mode="json")
    assert "dirty_count" in dumped["before_git_summary"]
    assert "dirty_count" in dumped["after_git_summary"]
    for key in (
        "branch",
        "head",
        "is_worktree",
        ...
        assert key not in str(dumped), f"Receipt leaked {key}"


def test_validate_git_state_extra_forbidden() -> None:
    """ValidateGitState rejects extra fields."""
    from pydantic import ValidationError

    from vibe.core.tools.builtins.validate import ValidateGitState
