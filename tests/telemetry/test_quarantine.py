...
def test_quarantine_schema_validates():
    ...
    extra_props = schema.get("properties", {}).keys()
    assert "debug_packet_count" in extra_props
    assert "quarantined_debug_packet_count" in extra_props
    assert "quarantine_file_path" in extra_props
    assert "quarantine_note" in extra_props
    # Verify types
...
def test_normal_event_goes_to_observability_not_quarantine(tmp_path: Path):
    import rig_relay.core.paths._vibe_home as vh
    from rig_relay.core.telemetry.local import log_local_event
    ...
    try:
        ...
        assert not quarantine_file.exists(), (
            "normal event should NOT be in quarantine"
        )
        assert observability_file.exists(), (
            "normal event should be in observability"
        )
    finally:
        vh.SESSIONS_ROOT._resolver = original_resolver
