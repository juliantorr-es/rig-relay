...
def test_security_packet_runner_plan_artifact_validates_and_stays_content_light():
    ...
    assert report["schema_version"] == "rig.github.security_packet_runner_plan.v1"
    assert report["content_light"] is True
    assert report["remote_mutation"] is False
    assert isinstance(report["plan_items"], list)
