...
def test_surface_audit_artifact_validates_and_stays_content_light():
    ...
    assert report["schema_version"] == "rig.github.surface_audit.v1"
    assert report["content_light"] is True
    assert report["remote_mutation"] is False
    assert isinstance(report["audited_surfaces"], list)
    assert isinstance(report["proposal_packets"], list)
