...
def test_cli_refusal_on_live_dirty_without_manifest():
    ...
    with TemporaryDirectory() as td:
        ...
        cmd = [sys.executable, "-m", "rig_relay.review_projection.cli", "project", "--repo-root", str(root)]
...
def test_cli_default_dry_run_and_candidate_emission():
    with TemporaryDirectory() as td:
        ...
        # 1. Dry run (default)
        cmd_dry = [
            sys.executable, "-m", "rig_relay.review_projection.cli", "project", 
            "--repo-root", str(root), 
        ...
        cmd_verify = [
            sys.executable, "-m", "rig_relay.review_projection.cli", "verify",
            "--zip-path", str(zips[0])
        ]
