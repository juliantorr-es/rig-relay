...
def test_adversarial_confidential_holdback():
    ...
    with TemporaryDirectory() as td:
        ...
        rules_path.write_text(json.dumps({
            "confidential_paths": ["secret_mechanism.py"]
        }))
...
def test_secret_bearing_fixture_refused():
    with TemporaryDirectory() as td:
        ...
        assert classifier.classify_file(env_file) == FileClassification.EXCLUDED_SECRET_OR_PRIVATE_MATERIAL
...
def test_public_baseline_offline_attestation():
    with TemporaryDirectory() as td:
        ...
        manifest = InclusionManifest(
            mode=ProjectionMode.PUBLIC_BASELINE_REVIEW,
        )
        ...
        assert classifier.classify_file(pub_file) == FileClassification.PUBLIC_ALREADY_DISCLOSED
        
        # Check tampering fails closed
        tampered_file = root / "public2.py"
        tampered_file.write_text("def public2(): pass")
        assert classifier.classify_file(tampered_file) == FileClassification.UNCLASSIFIED_REFUSED
