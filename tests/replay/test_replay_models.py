...
class TestReplayEvent:
    ...
    def test_kind_from_event_name(self) -> None:
        ...
        assert (
            _event_kind_from_event_name("rig.relay.tool_receipt.captured")
            == ReplayEventKind.RECEIPT
        )
        assert (
            _event_kind_from_event_name("rig.relay.governance.decision")
            == ReplayEventKind.GOVERNANCE_DECISION
        )
        assert (
            _event_kind_from_event_name("rig.relay.session.started")
            == ReplayEventKind.SESSION_EVENT
        )
