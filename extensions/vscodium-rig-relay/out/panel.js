...
exports.RigControlProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const client_1 = require("./client");
class RigControlProvider {
    _context;
    static viewType = 'rig-relay-control';
    _view;
    _client;
    ...
    async connect() {
        ...
        if (discovery) {
            ...
            const host = config.get('daemon.host') || '127.0.0.1';
            const port = config.get('daemon.port') || 5000;
            url = `ws://${host}:${port}`;
            token = await this._context.secrets.get('rig-relay.daemon.token');
        }
        if (!token) {
        ...
        this._client = new client_1.RigDaemonClient(url, token, this);
    ...
    onMessage(msg) {
        ...
        if (msg.schema === 'rig.ws.server.warning.v1') {
            this._view?.webview.postMessage({ type: 'warning', value: msg.message });
        }
    }
    onSnapshot(data) {
...
exports.RigControlProvider = RigControlProvider;
function getNonce() {
    let text = '';
